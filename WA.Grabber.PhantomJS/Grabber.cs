        ...
        public List<WeatherInfo> GetWeatherInfoList(CityInfo city)
        {
            ...
            using (var driver = new PhantomJSDriver())
            {
                foreach (var day in _days)
                {
                    ...
                    var currDate = DateTime.ParseExact(date, new[] { "d MMM", "d MMM y", "d MMM yyyy" }, CultureInfo.GetCultureInfo("ru"), DateTimeStyles.None);
                    var detailedWeather = new DetailedWeather() { WeatherByHours = new SortedDictionary<TimeSpan, HourDetails>(wlist.ToDictionary(x => x.Time)) };
