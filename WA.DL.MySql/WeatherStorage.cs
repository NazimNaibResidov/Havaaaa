...
namespace WA.DL.MySql
{
    public class WeatherStorage : IStorage
    {
        ...
        private string quers = $"CONVERT(BINARY(CONVERT(col USING latin1)) USING utf8mb4)";
        private static string helper(string data)
        {
            var result = $"CONVERT(BINARY(CONVERT('{data}' USING latin1)) USING utf8mb4)";
            string question = $"insert into cities (name, outerid, url) values (@name, @oid, @url)";
            return result;
        }
